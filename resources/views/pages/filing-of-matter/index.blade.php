...
<div class="row">
    <div class="col-12 col-md-5">
        <div class="card border-0">
            <div class="card-body">
                <form id="form" action="{{route('filing-of-matter.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div id="method"></div>
                    ...
                    <div class="form-label-wrapper">
                        ...
                        <input type="file" name="icon" id="icon" value="{{old('icon')}}" class="form-input">
                    </div>
                    ...
                    <tbody>
                        @foreach ($filingOfMatters as $filingOfMatter)
                            <tr>
                                ...
                                <td>
                                    <img src="{{url('storage').'/'.$filingOfMatter->icon}}" class="img-fluid" width="60">
                                </td>
                                <td>
                                    <form action="{{route('filing-of-matter.destroy',$filingOfMatter->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    <button class="btn text-danger" onclick="return confirm('Apakah anda yakin ingin menghapus data ini?');">
                                        <i class="fas fa-trash"></i>
                                        Hapus
                                    </button>
                                </form>
                                <button class="btn text-primary btn-edit" data-name="{{$filingOfMatter->name}}" data-price="{{$filingOfMatter->price}}" data-description="{{$filingOfMatter->description}}" data-icon="{{url('storage').'/'.$filingOfMatter->icon}}" data-url="{{route('filing-of-matter.update',$filingOfMatter->id)}}">
